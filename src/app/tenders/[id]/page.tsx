...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  return { title: tender?.name ?? "الفرصة" };
}
...
export default async function TenderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fields = [
    ...
    ["الحالة", statusLabel(tender.status)], ["تاريخ الترسية", formatDate(tender.award?.awardDate)], ["قيمة الترسية", tender.award ? currency.format(tender.award.amount) : "—"],
  ];
  return (
